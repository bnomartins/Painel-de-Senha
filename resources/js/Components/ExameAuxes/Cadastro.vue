...
<button
    class="my-2 mx-4 px-2 bg-transparent border border-indigo-700 text-indigo-700 hover:bg-indigo-600 hover:text-white rounded shadow-lg text-xl font-bold"
    @click="abrirModal"
  >
    Novo
</button>
...
<Modal :show="confirmar" @close="closeModal">
  ...
  <div class="mx-2 px-6 rounded text-xl w-full bg-gray-100">
    ...
    <form @submit.prevent="cadastrarExameAux()" class="w-full">
        <div class="m-2 p-2 rounded">
            <label class="font-bold text-xl text-gray-600" for="exame">Exame</label>
...
// Funções do Módulo ExameAux
  function cadastrarExameAux(){

    if(form.exmae != '' && form.descricao != ''){
