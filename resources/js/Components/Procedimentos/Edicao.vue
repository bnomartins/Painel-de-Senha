<template>
  ...
  <Modal :show="confirmar" @close="closeModal">
    ...
    <div class="py-8 border-b border-solid border-slate-200 rounded-t text-gray-600">
      <h1 class="text-center m-auto text-2xl font-bold" >
        Atendimento Ambulatorial - Procedimentos
      </h1>
    </div>
    ...
    <div class="mx-2 px-6 rounded text-md w-full bg-gray-100">
      ...
      <form @submit.prevent="editarProcedimento()" class="w-full">
          <div class="m-2 p-2 rounded">
              <label class="font-bold text-md text-gray-600" for="procedimento">Procedimento</label>
              <input type="text" placholder="Digite o procedimento" v-model="form.procedimento"
  ...
  const props = defineProps({
    ...
    const form = useForm({
      ...
      procedimento: props.procedimento.procedimento,
      descricao:  props.procedimento.descricao,
      disponivel:  props.procedimento.disponivel,

  })

function editarProcedimento(){
  if(form.procedimento != '' && form.descricao != ''){
    form.put(route('procedimentos.update', form.id),{
        preserveState: true,
        preserveScroll: true,
