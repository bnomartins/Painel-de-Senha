<template>
    ...
    <Modal :show="confirmar" @close="closeModal">
      ...
      <div class="py-8 border-b border-solid border-slate-200 rounded-t text-gray-600">
        <h1 class="text-center m-auto text-2xl font-bold" >
          Os dados do usuário serão removidos permanentemente, você deseja continuar?
        </h1>
      </div>
        ...
        <div class="mx-2 px-6 border rounded text-xl w-full">
            ...
            <form @submit.prevent="excluirUnidade(form.id)" class="w-full">
    ...
    function excluirUnidade(id){
        form.delete(route('unidade.destroy',  form.id),{
            onSuccess: () => (
              closeModal()
