<template>
  ...
  <Modal :show="confirmar" @close="closeModal">
      ...
      <div id="print" class="p-5 uppercase">
        ...
        <span style="font-size:10px;display:block;text-align:center;text-decoration:underline">___________________________________________________________________________________________________________________________________</span>
        <span style="font-size:25px;display:block;font-weight:bold;text-align:center">Assinatura do Paciente / Responsável</span>
        ...
        <div style="border: 1px solid black; padding:5px;line-height:20px; margin-bottom:10px;font-weight:bold;font-size:20px;text-align:justify" v-if="tipo == 'Atestado Médico'">
          <p>
        ...
        <p style="border: 1px solid black; padding:5px;line-height:15px;font-size:15px">Assinatura digital: ICP Brasil dfsdfsdfadsfdsafdfdf465d4fd654f3s5d4f684df65d4f654sd6f54sda6f54ds6a5f4d6f54ads65f465d4fa65
          f4d65f4s65d4f65das4f654d65f4d65f4d6544</p>
        <p style="text-align:center">Este documento pode ser verificado no site https://assinaturadigital.iti.gov.br</p>
      </div>
...
const print = () => {
  paperize()
    ...
    const refConselho = ref(props.atendimento[0].clinico[0].user.prestador[0].conselho+""+props.atendimento[0].clinico[0].user.prestador[0].conselho_estado);
    ...
    const replaceWithBr = () => {
      ...
      refConteudo.value = refConteudo.value.replace('#HRATENDINICIO#', moment(props.atendimento[0].created_at).format('hh:mm'))
      refConteudo.value = refConteudo.value.replace('#HRATENDFIM#', " ÀS "+moment(props.atendimento[0].updated_at).format('hh:mm'))
