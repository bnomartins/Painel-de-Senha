<template>
    ...
    <div class="mx-2 px-6 rounded text-md w-full bg-gray-100">
      ...
      <form @submit.prevent="editarExameAux()" class="w-full">
          <div class="m-2 p-2 rounded">
              <label class="font-bold text-md text-gray-600" for="exame">Coódigo</label>
              <input type="text" placholder="Digite o exame" v-model="form.codigo"
          ...
          <div class="m-2 p-2 rounded">
              <label class="font-bold text-md text-gray-600" for="descricao">Procedimento</label>
              <input type="text" placholder="Digite o descricao" v-model="form.procedimento"
              class="w-full rounded h-12 text-md text-gray-500 font-bold"/>
              <div class="bg-red-100 my-2 px-5 text-red-700 text-bold font-semibold text-md">
  ...
  const props = defineProps({
    ...
    const form = useForm({
      id: props.ambulatorioAux.id,
      codigo: props.ambulatorioAux.codigo,
      procedimento:  props.ambulatorioAux.procedimento,
      disponivel:  props.ambulatorioAux.disponivel,

  })

function editarExameAux(){
  if(form.id != '' && form.procedimento != ''){
    form.put(route('ambulatorioAuxes.update', form.id),{
        preserveState: true,
        preserveScroll: true,
