<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            Apoio ao Diagnóstico - Ambulatório
        </template>
        ...
        <div class="m-auto bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="m-5 border-b border-gray-200">
                ...
                <div class="p-2 m-2 flex justify-end">
                    <input
                        v-model="form.search"
                        @keyup.enter="pesquisarAmbulatorioAux"
                        type="text"
                    ...
                    <div class="inline-flex rounded-md shadow-sm" role="group">
                        ...
                        <button type="button" class="px-3 my-2 text-md font-bold text-white bg-gray-700 hover:bg-gray-500 border border-gray-200 rounded-r-md"
                          @click="limparPesquisaAmbulatorioAux()"
                          :disabled="form.processing && form.searchClear "
                          :class="{'opacity-25': form.processing && form.searchClear}"
                        >
                            <span v-if="form.processing && form.searchClear">Aguarde</span>
                    ...
                    <!-- <CadastroAmbulatorioAux  /> -->
                    

                    <!-- <Edicao :ambulatorioAux='form.ambulatorioAux' v-if="form.modalEditar"/> -->
                        ...
                        <div class="bg-gray-100 shadow mx-auto">
                                ...
                                <tbody class="bg-grey-light flex flex-col items-left justify-between overflow-y-scroll w-full" style="height: 20vh;">
                                    ...
                                    >
                                        ...
                                        <td class="text-right">
                                            ...
                                            <button class="ml-5 px-2 bg-blue-500 font-bold text-white rounded-md" @click.prevent="ativarGrupo(g.grupo, g.descricao)" v-if="ambulatorioAuxs ? !ambulatorioAuxs.includes(g.grupo) : false">Ativar</button> 
                                            <button class="ml-5 px-2 bg-red-500 font-bold text-white rounded-md" @click.prevent="desativarGrupo(g.grupo)" v-if="ambulatorioAuxs ? ambulatorioAuxs.includes(g.grupo) : false">Desativar</button> 
                                        </td>
                                    </tr>
                            ...
                            <tr v-for="(p, index) in procedimentos.data" :key="p.id"
                                ...
                                <td class="px-6 whitespace-nowrap">
                                    ...
                                    <div class="flex">
                                        <div class="flex-col">
                                            <span v-if="ambulatorioAuxs ? (ambulatorioAuxs.includes(p.codigo.substring(0,5))) : false">
                                ...
                                <td class="px-2 py-2 text-md float-right">
                                    ...
                                    <EdicaoAmbulatorioAux  :ambulatorioAux="{id: p.id, codigo: p.codigo, procedimento: p.procedimento,  disponivel: p.disponivel}" />
...
import CadastroAmbulatorioAux from '../../Components/AmbulatorioAuxes/Cadastro.vue'
import EdicaoAmbulatorioAux from '../../Components/AmbulatorioAuxes/Edicao.vue'
import ExcluirAmbulatorioAux from '../../Components/AmbulatorioAuxes/Excluir.vue'
...
function statusToggle(disponivel, id){
    ...
    form.post(route('ambulatorioAuxes.store'), {
        only: ['procedimentos'],
...
function pesquisarAmbulatorioAux(){
    form.searchClear = false
    form.get(route('ambulatorioAuxes.index'), {
        search: form.search
    })
}
...
function limparPesquisaAmbulatorioAux(){
